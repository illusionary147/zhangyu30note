...
#let 大数定律与中心极限定理 = {
  ...
  section[大数定律]
  [
    + 切比雪夫大数定律：假设${X_n}$是相互独立的随机变量序列，如果方差$D X_i$存在且一致有上界，即存在常数$C$，使$D X_i<=C$对一切$i>=1$均成立，则${X_n}$服从大数定律：$ 1/n sum_(i=1)^n X_i arrow.long^P 1/n sum_(i=1)^n E X_i $
    ...
    + 辛钦大数定律：假设${X_n}$是独立同分布的随机变量序列，如果数学期望$E X_i=mu$存在，则$1/n sum_(n arrow oo)X_i arrow.long^P mu$,即对任意实数$epsilon>0$,有$ lim_(n arrow oo)P{abs(1/n sum_(i=1)^(n)-mu)<epsilon}=1 $
  ]
  section[中心极限定理]
  [
    + 列维-林德伯格定理：假设${X_n}$是独立同分布的随机变量序列，如果$ E X_i =mu,D X_i=sigma^2>0 $存在，则对任意实数$x$有$ lim_(n arrow oo)P{(sum_(i=1)^n X_i - n mu)/(sqrt(n)sigma)<=x}=1/sqrt(2 pi)integral_(-oo)^x e^(-t^2/2)dd(t)=Phi(x) $
